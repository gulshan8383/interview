...
exports.getAllStudentUsers = async (req, res) => {
  try {
    const studentUsers = await User.find({ accountType: "Student" })
      .populate("additionalDetails")
      .exec();
...
exports.getAllInstructorUsers = async (req, res) => {
  try {
    const instructorUsers = await User.find({ accountType: "Instructor" })
      .populate("additionalDetails")
      .exec();

    res.status(200).json({
      success: true,
      message: "Instructor Users fetched successfully",
      data: instructorUsers,
      count: instructorUsers.length,
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      success: false,
      message: "Failed to fetch instructor users",
      error: error.message,
    });
  }
};
